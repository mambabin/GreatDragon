...
static inline void ShowHelp(const char *me){
	printf("Usage: %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n", me, SKILL_XML, STATUS_XML, EQUIPMENT_XML, GOODS_XML, NPC_XML, PET_XML, MISSION_XML, BUSINESS_XML, BLOOD_XML, BLOODNODES_XML, BOXES_XML, EXPLORE_XML, GUIDENODE_XML, GUIDEPASS_XML, AWARD_XML, DESIGNATION_XML, VIP_XML, FASHION_XML, MAPINFO_XML, EQUIPRECIPE_XML, BASEWING_XML, WING_XML, ACTIVE_XML, NOTICE_XML, MAILGIFT_XML, SOCIATY_XML, SOCIATYSKILL_XML, PLAYOFF_XML, TRANSFORMS_XML, DROPTABLES_XML, HELPERNODE_XML, HELPERGROUP_XML, PETHALO_XML, EXCHANGETABLE_XML, GODSHIP_XML, RESERVATION_XML, RIDESINFO_XML);
}
