...
static void ResetTransformCount(Fight *fight)
{
	...
	if(itemPackage == NULL)
		fight->transformCount = Config_TransformCount(0);
	else
		fight->transformCount = Config_TransformCount(itemPackage->vip());
}
...
static void ModifyLevel(struct Fight *fight, int delta, bool prepare) {
	...
	switch(baseAtt->professionType()) {
		case ProfessionInfo::KNIGHT:
			{
				...
				Fight_ModifyProperty(fight, FightAtt::MAXHP, delta * 120);
				Fight_ModifyProperty(fight, FightAtt::ACCURACY, delta * 3);
		...
		case ProfessionInfo::MAGICIAN:
			{
				Fight_ModifyProperty(fight, FightAtt::ATK, delta * 11);
				Fight_ModifyProperty(fight, FightAtt::DEF, delta * 5);
				Fight_ModifyProperty(fight, FightAtt::MAXHP, delta * 100);
				Fight_ModifyProperty(fight, FightAtt::ACCURACY, delta * 3);
				Fight_ModifyProperty(fight, FightAtt::CRIT, delta * 4);
				Fight_ModifyProperty(fight, FightAtt::DODGE, delta * 3);
			}
			break;
		default:
		...
		if (fight->component->player != NULL) {
			...
			if (info != NULL) {
				if (info->mapType() == MapInfo::PRACTICE || info->mapType() == MapInfo::PVP) {
					if (PlayerPool_ReservationMap() == map) {
						int64_t roleID = fight->component->baseAtt->roleID();
	...
	if (mapInfo->id() == Config_FactionWarMap()) {
		...
		if (fight->component->player != NULL) {
			if (mapID == PlayerPool_ReservationMap()) {
				PlayerPool_ReservationRefresh(-4, PlayerEntity_RoleID(killer->component->player));
			}
			Fight_ClearRoom(fight, false, 0);
...
int Fight_Transform(struct Fight *fight, bool transform) {
	...
	if (!fight->isTransforming && transform) {
		...
		if(fight->component->player != NULL)
		{
			if(fight->transformCount <= 0)
				return -1;
		}

		fight->transformCount--;
	...
	if (mapInfo->mapType() == MapInfo::HELL) {
		...
		if (count1 + count2 + count3 >= 2) {
			...
			if (another == NULL) {
				...
				int64_t awardPVPScore = 0;
				if (win) {
					if (mapInfo->mapType() == MapInfo::PVP) {
						if (fight->component->player != NULL) {
							Item_ModifyPKStatus(fight->component->item);

							int32_t diffLevel = fight->att->pkLevel() - another->att->pkLevel();
							int64_t awardPVPScore = Config_PKScore(true, diffLevel);
							Item_ModifyPKScore(fight->component->item, awardPVPScore);

							const ItemPackage *itemPackage = Item_Package(fight->component->item);
							int32_t pkLevel = Config_PKLevel(itemPackage->pkScore());
							fight->att->set_pkLevel(pkLevel);
							...
							if (Config_ScribeHost() != NULL) {
								...
								SNPRINTF2(buf, index, "\",\"roleName\":\"%s", fight->component->baseAtt->name());
								index += strlen(index);
								SNPRINTF2(buf, index, "\",\"pvpScore\":\"%lld\"}", (long long int)awardPVPScore);
...
*/
					...
					if (mapInfo->mapType() == MapInfo::PVP) {
						if (another->component->player != NULL) {
							Item_ModifyPKStatus(another->component->item);

							int32_t diffLevel = another->att->pkLevel() - fight->att->pkLevel();
							int64_t awardPVPScore = Config_PKScore(true, diffLevel);
							Item_ModifyPKScore(another->component->item, awardPVPScore);

							const ItemPackage *itemPackage = Item_Package(another->component->item);
							int32_t pkLevel = Config_PKLevel(itemPackage->pkScore());
							another->att->set_pkLevel(pkLevel);
...
int Fight_ClearRoom(struct Fight *fight, bool win, int group, NetProto_ClearRoom *award) {
	if (!Fight_IsValid(fight))
		return -1;
	int32_t dropMoney = fight->dropMoney;
	fight->dropMoney = 0;
	if (fight->component->player == NULL)
		return -2;
	if (fight->clearRoom)
		...
		if (mapInfo->id() == Config_WorldBossMap()
			...
			if (win) {
				...
				if (mapInfo->id() >= Config_BossBegin() && mapInfo->id() <= Config_BossEnd()) {
					...
					int count = fight->component->playerAtt->dayEvent(77) >> 16;
					PlayerEntity_SetDayEvent(fight->component->player, 77, fight->component->playerAtt->dayEvent(77) + ((count + 1) << 16));
				...
				{
					int index = mapInfo->id() - Config_RoomBegin();
					if(index < 0)
						index = 0;
					int events[] = {78, 79, 80};
					int m_index = index / 10;
					if(m_index < (int)(sizeof(events) / sizeof(events[0])))
					{
						m_index = events[m_index];
						int bit = index % 10 * 3;
						int count = (fight->component->playerAtt->dayEvent(m_index) & (7 << bit)) >> bit;
						PlayerEntity_SetDayEvent(fight->component->player, m_index, (fight->component->playerAtt->dayEvent(m_index) & (~(7 << bit))) | ((count + 1) << bit));
					}
				...
				if (mapInfo->id() == Config_SurviveMap()) {
					...
					if (group > 0) {
						...
						if ( ((fight->component->playerAtt->dayEvent(30) & 0xF00) >> 8) >= 2 ) {
							const VIPInfo *vip = VIPInfoManager_VIPInfo(fight->component->playerAtt->itemPackage().vip());
							int count = fight->component->playerAtt->dayEvent(77) & 0xFFFF;
							if (vip == NULL) {
								return -5;
							}
							if (count >= vip->survive()) {
								return -5;
							}
							int surviveRmb = 20 + 30 * count;
							if (!Item_HasRMB(fight->component->item, surviveRmb))
								return -5;
							Item_ModifyRMB(fight->component->item, -surviveRmb, false, 37, 0, 0);
						}
						...
						if ( ((fight->component->playerAtt->dayEvent(30) & 0xF00) >> 8) < 2 ) {
							...
							int count = fight->component->playerAtt->dayEvent(77) & 0xFFFF;
							PlayerEntity_SetDayEvent(fight->component->player, 77, (fight->component->playerAtt->dayEvent(77) & 0xFFFF0000) | (count + 1));
						}
					} else {
						money = 0;
						exp = 0;
				...
				int expPercent = GmSet_GetPercent(GmSet_Type_EXP, mapInfo->id());
				if(expPercent != -1)
					exp = int(exp * expPercent / 100.0f);
				Fight_ModifyExp(fight, exp);
				int moneyPercent = GmSet_GetPercent(GmSet_Type_COIN, mapInfo->id());
				if(moneyPercent != -1)
					money = int(money * moneyPercent / 100.0f);
...
int Fight_OpenRoomBox(struct Fight *fight, bool free, int32_t *results, size_t size) {
	...
	int dropPercent = GmSet_GetPercent(GmSet_Type_DROP, mapInfo->id());
	if(dropPercent != -1)
	{
		int dropRate = int(dropPercent / 100.0f);
		for(int i = 0; i < dropRate; i++)
		{
			for(int j = 0; j < ret; j++)
			{
				results[ret * i + j] = results[j];
			}
		}
		ret *= dropRate;
	}
...
int Fight_QuickFight(struct Fight *fight, int32_t room, int32_t count) {
	...
	if (room >= Config_TowerBegin() && room <= Config_TowerEnd()) {
		...
		int dayEvent = fight->component->playerAtt->dayEvent(35);
		if ((dayEvent & 0xFF) == 0xFF) {
			return -1;
		}else {
			PlayerEntity_SetDayEvent(fight->component->player, 35, dayEvent + 1, false);
	...
	if (map == Config_PVPMap()) {
		if (fights.size() < 2) {
			...
			if (atts.size() > 0) {
				...
				int rand = Time_Random(0, 100);
				float fRatio = 0.0f;
				int pkLevel = fights[0]->att->pkLevel();
				int level = fights[0]->att->level();
				if (rand < 30) {
					fRatio = 0.9f;
					if (pkLevel > 1 || pkLevel <= 10)
						pkLevel--;
				...
				playerAtt->mutable_att()->mutable_fightAtt()->set_pkLevel(pkLevel);
				playerAtt->mutable_att()->mutable_fightAtt()->set_level(level);
				bool male = playerAtt->att().baseAtt().male();
				string strName = Config_RandomName(male);
				playerAtt->mutable_att()->mutable_baseAtt()->set_name(strName.c_str());

				NPCAtt att;
				PlayerEntity_CopyAsNPC(playerAtt, &att, fRatio, false);
...
static int UpdateWaitPVP(set<struct Fight *, PVPComp> &pool, size_t num, int32_t map, bool pvp, struct Fight * self) {
	if (pool.size() < num)
		return 0;
	...
	for (set<struct Fight *, PVPComp>::iterator it = pool.begin(); it != pool.end(); it++) {
		...
		if (pvp) {
			if ((*it) != self) {
				if (abs(self->att->pkLevel() - (*it)->att->pkLevel()) <= 1) {
...
int Fight_BeginWaitPVP(struct Fight *fight) {
	...
	return UpdateWaitPVP(package.pvp, 2, Config_PVPMap(), true, fight);
}
...
int Fight_BeginWaitHell(struct Fight *fight) {
	...
	return UpdateWaitPVP(package.hell, Config_MaxPlayersOfLineInWorldBoss(), Config_HellMap(), false, NULL);
}
...
int Fight_StrongBaseWing(struct Fight *fight, NetProto_StrongBaseWing* proto) {
	if (!Fight_IsValid(fight))
		return -1;
	...
	for (int i = 0; i < info->goods_size(); ++i) {
		DEBUG_LOG("i :%d", i);
		DEBUG_LOG("goodsid :%d", info->goods(i));
		int count = Item_Count(fight->component->item, ItemInfo::GOODS, info->goods(i));
		if (count < info->count(i)) {
			return -3; 
		} 
	}
	for (int i = 0; i < info->goods_size(); ++i) {
		Item_DelFromPackage(fight->component->item, ItemInfo::GOODS, (int64_t)info->goods(i), info->count(i));
	}

	int success = info->success() * 100;
	int dayEvent = fight->component->playerAtt->dayEvent(76) >> 16;
	int addWishes = Time_Random(0, 3) + 1;
	if (dayEvent < 303) {
		dayEvent += addWishes;
		if (dayEvent > 303)
			dayEvent = 303;
		PlayerEntity_SetDayEvent(fight->component->player, 76, (fight->component->playerAtt->dayEvent(76) & 0xFFFF) + (dayEvent << 16));
	}

	success += (int)dayEvent / 3.5;
	int rand = Time_Random(0, 100);
	level = fight->att->baseWingLevel();
	degree = fight->att->baseWingDegree();

	level = fight->att->baseWingLevel();
	degree = fight->att->baseWingDegree();
	proto->set_success(false);
	if (success >= rand) {
		info = EquipmentInfoManager_BaseWing(level + 1, 0);
		if (info == NULL) {
			degree++;
		}else {
			level++;
			degree = 0;
			PlayerEntity_SetDayEvent(fight->component->player, 76, fight->component->playerAtt->dayEvent(76) & 0xFFFF);
			proto->set_success(true);
		}
	}else {
		degree++;
	}
	proto->set_wishes(fight->component->playerAtt->dayEvent(76) >> 16);
...
bool Fight_RestCheckPoint(struct Fight *fight) {
	...
	if ((dayEvent & 0xFF) != 0) {
		...
		if (vip == NULL)
			return false;
		if (dayEvent >= vip->towerResetCount() + 1)
			return false;
		if ((dayEvent & 0xFF) == 0xFF)
			return false;
	}

	if (!Item_HasRMB(fight->component->item, dayEvent * 100))
		return false;
	Item_ModifyRMB(fight->component->item, -dayEvent * 100, false, 40, 0, 0);
...
int Fight_DropItem(struct Fight *fight, int npc, int index, int id) {
	...
	int32_t Fight_PetPower(const PB_PlayerAtt *att, int index) {
		...
		const static int addValue[PB_FightAtt_PropertyType_PropertyType_ARRAYSIZE] = {6, 4, 20, 1, 2, 2};
