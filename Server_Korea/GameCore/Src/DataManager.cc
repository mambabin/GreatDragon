...
static bool FlushLocalCache() {
	MYSQL *conn = GetShortConn();
	if(conn == NULL){
		return false;
	}
			...
			for (size_t i = 0; i < saving.size(); i++) {
				...
				if (SignalHandler_IsOver()) {
					if (unit->data->att().baseAtt().roleID() == CONFIG_EXIT_CODE) {
						if(!FlushLocalCache()){
							DEBUG_LOGERROR("Flush local error when close server.");
						}
						...
						MYSQL *conn = GetShortConn();
						if(conn == NULL){
							DEBUG_LOGERROR("Save role error when close server.");
							exit(EXIT_FAILURE);
						}
					...
					if (unit->insert) {
						MYSQL *conn = GetShortConn();
						if(conn == NULL){
							DEBUG_LOGERROR("Insert role error.");
							exit(EXIT_FAILURE);
						}
...
void DataManager_Init() {
	...
	if(!FlushLocalCache()){
		DEBUG_LOGERROR("Flush local error when start server.");
		exit(EXIT_FAILURE);
	}
...
static void FreeResultRecharge() {
	DEBUG_LOGERROR("f r r 1");
	if(package.resRecharge != NULL) {
	DEBUG_LOGERROR("f r r 2");
		mysql_free_result(package.resRecharge);
	DEBUG_LOGERROR("f r r 3");
		package.resRecharge = NULL;
	DEBUG_LOGERROR("f r r 4");
	}
	DEBUG_LOGERROR("f r r 5");
	int ret = mysql_ping(package.connRecharge);
	DEBUG_LOGERROR("f r r 6");
	if (ret != 0)
		DEBUG_LOGERROR("Failed to mysql ping, ret: %d", ret);
...
void DataManager_FilterRecharge(DCProto_FilterRecharge *proto) {
	...
	DEBUG_LOGERROR("d r 4");
	FreeResultRecharge();
	DEBUG_LOGERROR("d r 5");
...
static void QueryRecharge(void *arg) {
	...
	if (ret >= 0) {
		int re = DataManager_RechargeOver(unit->recharge.recharge().order().c_str(), &unit->recharge.info(), unit->recharge.roleID(), unit->recharge.level());
		if(re == 0)
		{
...
int DataManager_LoadRankRecord(DCProto_InitRank* rank) {
	...
	if (rank->type() == NetProto_Rank::POWER) {
		...
		SNPRINTF1(sql, "select id, name, occupational, level, power from %s where godRank is not NULL order by godRank asc", Config_RoleTable());
		...
		//SNPRINTF1(sql, "select roleID, `name` from %s where mapID = -24 and score > %d", Config_SingleTable(), (int)rank->finalKiller().arg1());
	} else if (rank->type() == NetProto_Rank::LUCK) {
		SNPRINTF1(sql, "select id, `name`, occupational, level, pvpScore, faction from %s where level is not null and pvpScore is not null order by luck pvpScore desc", Config_RoleTable());
	} else {
		return -2;
	}
	...
	for (MYSQL_ROW row = mysql_fetch_row(package.res); row != NULL; row = mysql_fetch_row(package.res)) {
		...
		if (rank->type() == NetProto_Rank::FACTION) {
			...
			string strJoint = "";
			if (row[1] != NULL)
				strJoint = string((const char *)row[1]);
			strJoint += "?";
			if (row[5] != NULL)
				strJoint += string((const char *)row[5]);
			info->mutable_role()->set_name(strJoint.c_str());
