...
static struct{
	...
	int equipInheritPercent;
	int equipRandomPercent2;
	int equipRandomPercent3;
	vector<string> firstName;
	vector<string> maleName;
	vector<string> femaleName;
}config;
...
void Config_Init() {
	...
	key = "equipInheritPercent";
	if (!ConfigUtil_ReadStr(file, key, value, sizeof(value))) {
		DEBUG_LOGERROR("Failed to read config field: %s", key);
		exit(EXIT_FAILURE);
	}
	config.equipInheritPercent = atoi(value);
	
	key = "equipRandomPercent2";
	if (!ConfigUtil_ReadStr(file, key, value, sizeof(value))) {
		DEBUG_LOGERROR("Failed to read config field: %s", key);
		exit(EXIT_FAILURE);
	}
	config.equipRandomPercent2 = atoi(value);

	key = "equipRandomPercent3";
	if (!ConfigUtil_ReadStr(file, key, value, sizeof(value))) {
		DEBUG_LOGERROR("Failed to read config field: %s", key);
		exit(EXIT_FAILURE);
	}
	config.equipRandomPercent3 = atoi(value);
	...
	{
		string path = Config_DataPath() + string("/first_name.txt");
		file = fopen(path.c_str(), "r");
		config.firstName.clear();
		if (file == NULL) {
			DEBUG_LOGERROR("Failed to open %s", path.c_str());
			exit(EXIT_FAILURE);
		}
		while (fgets(value, sizeof(value), file) != NULL) {
			config.firstName.push_back(value);
			string &str = config.firstName.back();
			if (str[str.length() - 1] == '\n')
				str.erase(str.size() - 1);
		}
		fclose(file);

		path = Config_DataPath() + string("/male_last_name.txt");
		file = fopen(path.c_str(), "r");
		config.maleName.clear();
		if (file == NULL) {
			DEBUG_LOGERROR("Failed to open %s", path.c_str());
			exit(EXIT_FAILURE);
		}
		while (fgets(value, sizeof(value), file) != NULL) {
			config.maleName.push_back(value);
			string &str = config.maleName.back();
			if (str[str.length() - 1] == '\n')
				str.erase(str.size() - 1);
		}
		fclose(file);

		path = Config_DataPath() + string("/female_last_name.txt");
		file = fopen(path.c_str(), "r");
		config.femaleName.clear();
		if (file == NULL) {
			DEBUG_LOGERROR("Failed to open %s", path.c_str());
			exit(EXIT_FAILURE);
		}
		while (fgets(value, sizeof(value), file) != NULL) {
			config.femaleName.push_back(value);
			string &str = config.femaleName.back();
			if (str[str.length() - 1] == '\n')
				str.erase(str.size() - 1);
		}
		fclose(file);
...
int Config_EquipInheritPercent() {
	return config.equipInheritPercent > 100 ? 100 : config.equipInheritPercent;
}

int Config_EquipRandomPercent2() {
	return config.equipRandomPercent2 > 100 ? 100 : config.equipRandomPercent2;
}

int Config_EquipRandomPercent3() {
	return config.equipRandomPercent3 > 100 ? 100 : config.equipRandomPercent3;
}
...
std::string Config_RandomName(bool male) {
	std::string strName = "";
	int fristNameIndex = Time_Random(0, (int)config.firstName.size());
	strName = config.firstName[fristNameIndex];

	if (male) {
		int lastNameIndex = Time_Random(0, (int)config.maleName.size());
		strName += config.maleName[lastNameIndex];
	}else {
		int lastNameIndex = Time_Random(0, (int)config.femaleName.size());
		strName += config.femaleName[lastNameIndex];
	}
	return strName;
}
