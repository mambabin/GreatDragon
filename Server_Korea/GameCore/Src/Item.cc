...
struct GodShipComp{
	...
	static int Compare(const void *lhs_, const void *rhs_) {
		...
		const GodShipAsset *assetlhs = &package->godShips(package->godShipsPackage(lhs));
		const GodShipAsset *assetrhs = &package->godShips(package->godShipsPackage(rhs));
		if (assetlhs->quality() < assetrhs->quality()) 
			return -1;
		if (assetlhs->level() < assetrhs->level())  
			return -1;
		if (assetlhs->id() > assetrhs->id())  
			return -1;
		return 1;
	}
...
void Item_ModifyRidesFood(struct Item *item, int64_t delta)
{
	if(!Item_IsValid(item))
		return;

	item->package->set_ridesFood(item->package->ridesFood() + delta);
	if(item->package->ridesFood() < 0)
		item->package->set_ridesFood(0);

	if(delta != 0)
	{
		static NetProto_ModifyRidesFood modifyRidesFood;
		modifyRidesFood.Clear();
		modifyRidesFood.set_ridesFood(item->package->ridesFood());
		int32_t id = PlayerEntity_ID(item->component->player);
		GCAgent_SendProtoToClients(&id, 1, &modifyRidesFood);
	}
	...
	if (Config_ScribeHost() != NULL) 
	{
		if (delta != 0) 
		{
			...
			index += strlen(index);
			SNPRINTF2(buf, index, "\",\"ridesFoodsChange\":\"%lld", (long long int)delta);
			index += strlen(index);
			SNPRINTF2(buf, index, "\",\"ridesFood\":\"%lld", (long long int)item->package->ridesFood());
			index += strlen(index);
			SNPRINTF2(buf, index, "\"}");
			int32_t player = PlayerEntity_ID(item->component->player);
			ScribeClient_SendMsg(ScribeClient_Format(AccountPool_IDToAccount(player), player, "RidesFoodChange", buf));
		}
	}
}
	...
	*cr.mutable_info() = *info;
	...
	switch(reason) {
		...
		case 36:
			...
			break;
		case 37:
			cr.set_reason("survive");
			break;
		case 38:
			cr.set_reason("hero");
			break;
		case 39:
			cr.set_reason("active fuben");
			break;
		case 40:
			cr.set_reason("tower");
			break;
		case 41:
			cr.set_reason("clear quickfight time");
			break;
	...
	if (delta > 0) {
		if (recharge) {
			...
			if (reason == -1) {
				...
				int32_t clock = Time_TodayZeroTime(Config_OpenTime(NULL));
				if ((Time_TimeStamp() - clock) >= 0 && (Time_TimeStamp() - clock) < 3600 * 24 * 7) {
					PlayerEntity_SetFixedEvent(item->component->player, 113, item->component->playerAtt->fixedEvent(113) + delta);
				}
...
void Item_ModifyOpenServerScore(struct Item *item, int32_t delta) {
	...
	item->package->set_openServerScore(item->package->openServerScore() + delta);
	if (item->package->openServerScore() < 0)
		item->package->set_openServerScore(0);

	if (delta != 0) {
		static NetProto_ModifyOpenServerScore modifyOpenServerScore;
		modifyOpenServerScore.Clear();
		modifyOpenServerScore.set_score(item->package->openServerScore());
		int32_t id = PlayerEntity_ID(item->component->player);
		GCAgent_SendProtoToClients(&id, 1, &modifyOpenServerScore);
	}
}
...
int Item_GenRides_Whole(struct Item *item, int32_t id, int32_t count, NetProto_GetRes *gr)
{
	...
	const RidesInfo *ridesInfo = RidesInfoManager_RidesInfo(id, 0);
	if(ridesInfo == NULL)
		return -2;
	...
	int num = 0;
	RidesAsset m_asset;
	for(int i = 0, count1 = item->package->rides_size(); i < count1; i++)
	{
		m_asset = item->package->rides(i);
		if(m_asset.model() < 0)
		{
			arr[num] = i;
			num++;
	...
	if(gr != NULL)
	{
		...
		itemInfo->set_type(PB_ItemInfo::RIDES);
	...
	for(int i = 0; i < num; i++)
	{
		...
		for(int j = 0, count1 = asset->lockAtt_size(); j < count1; j++)
		{
	...
	DEBUG_LOGERROR("Rides to mail->%d, %d", num, count);
	if(num > 0)
	{
		PlayerEntity_AddMail(item->component->player, ItemInfo::RIDES, id, num, Config_Words(0), Config_Words(100));
	}
...
int32_t Item_AddEquip(struct Item *item, const EquipmentInfo *info) {
	...
	for (int i = 0; i < item->package->equips_size(); i++) {
		...
		if (asset->mode() < 0) {
			...
			int randomType[] = {-1, -1, -1};
			int randomData[] = {-1, -1, -1};
			EquipmentInfoManager_GetRadomData(
					info->equipmentLevel(),
					info->colorType(),
					info->type(),
					randomType,
			...
			for(int j = 0; j < 3; j++)
			{
				asset->set_randomType(j, randomType[j]);
				asset->set_randomDelta(j, randomData[j]);
			}
			int effectId = EquipmentInfoManager_GetRadomEffect(info->randomEffect());
			asset->set_effectId(effectId);
...
static void GetBoxRes(struct Item *item, BoxInfo::Type type, int id, int count, NetProto_GetRes *gr, int boxID) {
	switch(type) {
		case BoxInfo::EXP:
			...
			break;
		case BoxInfo::RIDES_FOOD:
			{
				Item_ModifyRidesFood(item, id);
				PB_ItemInfo *item = gr->add_items();
				item->set_type(PB_ItemInfo::RIDES_FOOD);
				item->set_count(id);
			}
			break;
		case BoxInfo::RIDES:
			{
				Item_GenRides_Whole(item, id, count, gr);
			}
			break;
		case BoxInfo::MONEY:
			{
...
int Item_GetGift(struct Item *item, AwardInfo::Type type, int index, int arg) {
		DEBUG_LOG("DDDDDDDDD %d", (int)type);
	if (!Item_IsValid(item))
		return -1;
	...
	} else if (type == AwardInfo::OPENSERVERSCORE) {
		int value = item->component->playerAtt->fixedEvent(114);
		DEBUG_LOG("DDDDDDDDD %d", value);
		if (value & (1 << index))
			return -1;
	} else {
		return -1;
	...
	if (type == AwardInfo::ONLINE) {
		...
		PlayerEntity_SetFixedEvent(item->component->player, 114, item->component->playerAtt->fixedEvent(114) | (1 << index));
	} else {
		return -1;
	}
	return 0;
...
int Item_TransformLevelUp(struct Item* item, int id, bool activaction) {
	...
	for (int i = 0; i < itemInfo->materials_size(); ++i) {
		DEBUG_LOG("goodsid: %d", itemInfo->materials(i));
		DEBUG_LOG("i: %d", i);
...
void Item_Swallow(struct Item *item, int32_t index, int32_t select) {
	...
	if (index == 1) {
		...
		if (item->component->player != NULL) {
			...
			if (att == NULL)
				return;
			for (int j = 0; j < att->godShips_size(); j++) {
				if (att->godShips(j) == select) {
					i = j;
					break;
				}
			}
		...
		for (int i = 0; i < item->package->godShipsPackage_size(); i++) {
			if (item->package->godShipsPackage(i) == select)
				continue;
			if (item->package->godShipsPackage(i) != -1) {
				GodShipAsset *asset = item->package->mutable_godShips(item->package->godShipsPackage(i));
				if (asset == NULL)
...
//int a[5] = {4, 6, 2, 5, 3};
//qsort(&a[0], 5, sizeof(int), GodShipComp::Compare);
//	qsort(&(item->package->godShipsPackage(0)), item->package->godShipsPackage_size(), sizeof(int), GodShipComp::Compare);
...
DEBUG_LOG("qualityi : %d", asseti->quality());
DEBUG_LOG("qualityj : %d", assetj->quality());
if (asseti->quality() < assetj->quality()) {
...
for (i = index; i < item->package->godShipsPackage_size() - 1; i++) {
	DEBUG_LOG("indexi(%d)start", i);
	int pooli = item->package->godShipsPackage(i);
	if (pooli == -1)
		return;
	...
	for (j = i + 1;j < item->package->godShipsPackage_size(); j++) {
		...
		if (asseti->level() != assetj->level()) {
			DEBUG_LOG("leveli : %d", asseti->level());
			DEBUG_LOG("levelj : %d", assetj->level());
			if (asseti->level() < assetj->level()) {
				m = item->package->godShipsPackage(i);
				DEBUG_LOG("index(%d) : %d", i, m);
				item->package->set_godShipsPackage(i, item->package->godShipsPackage(j));
...
for (i = index; i < item->package->godShipsPackage_size() - 1; i++) {
	...
	for (j = i + 1;j < item->package->godShipsPackage_size(); j++) {
		...
		if (asseti->id() != assetj->id()) {
			DEBUG_LOG("idi : %d", asseti->id());
			DEBUG_LOG("idj : %d", assetj->id());
			if (asseti->id() < assetj->id()) {
				m = item->package->godShipsPackage(i);
				DEBUG_LOG("index(%d) : %d", i, m);
				item->package->set_godShipsPackage(i, item->package->godShipsPackage(j));
...
int Item_RidesTrain(struct Item *item, int32_t index, bool high)
{
	...
	/*
	   ...
	   */
	int down = Time_RandomEx(0, canDown);
	down = downIndex[down];
	int up = -1;
	int num = 0;
	while(true)
	{
		up = Time_RandomEx(0, 6);
		num++;
		if(down == up)
			continue;
		else
...
int Item_RidesInherit(struct Item *item, NetProto_RidesInherit proto)
{
	...
	{
		DEBUG_LOGERROR("rides expend size->%d", proto.ridesExpend_size());
		return -2;
	}
	...
	for(int i = 0, count = proto.ridesExpend_size(); i < count; i++)
	{
		...
		if(ridesAsset == NULL)
		{
			DEBUG_LOGERROR("rides index->%d", i_index);
			return -4;
		}
		if(ridesAsset->model() < 0)
			return -5;
...
int Item_RidesUP(struct Item *item, NetProto_RidesUP proto)
{
	...
	DEBUG_LOGERROR("rides up->%d, %d, %d", info->tpStar(), asset->star(), proto.star());
...
int Item_RidesLockAtt(struct Item *item, NetProto_RidesLockAtt proto)
{
	...
	if(proto.lockIndex() < 0 || proto.lockIndex() >= asset->lockAtt_size())
		return -4;
	...
	if(!asset->lockAtt(proto.lockIndex()))
	{
		int v = (int)(info->att(proto.lockIndex()) * 0.3);
		if(asset->att(proto.lockIndex()) - 5 < v)
			return -6;

		int num = 0;
		for(int i = 0, count = asset->lockAtt_size(); i < count; i++)
		{
			if(i == proto.lockIndex())
				continue;
			if(asset->lockAtt(i))
				num++;
			else
			{
				v = (int)(info->att(i) * 0.3);
				if(asset->att(i) - 5 < v)
					num++;
			}
		}
	...
	asset->set_lockAtt(proto.lockIndex(), !asset->lockAtt(proto.lockIndex()));

	proto.set_lock(asset->lockAtt(proto.lockIndex()));
...
void Item_ResetPvpScore(struct Item *item) {
	...
	item->package->set_pkScore(1000);
	item->package->set_pkStatus(false);
}
...
void Item_ModifyPKStatus(struct Item *item) {
	...
	item->package->set_pkStatus(true);
}
