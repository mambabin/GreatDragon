...
static struct{
	...
	vector<RecordInfo> rankPvp;
	map<int64_t, int64_t> rankPvpInfo;

	int32_t rankPvpRecordsTime;
...
//TODO:
	static int reservationInterval = 0;
	if ( reservationInterval == Time_TimeStamp() ) {
		return;
	}

	reservationInterval = Time_TimeStamp();
	...
	for (int index = 0; index < 20; ++index) {
		{
			...
			if (Time_TimeStamp() == begin) {

				{
					std::vector<struct PlayerEntity *> players;

					PlayerPool_Players(PlayerPool_ReservationMap(), &players);
						...
						if (att1 != NULL && att2 != NULL) {
							if ( att1->att().movementAtt().mapID() == PlayerPool_ReservationMap() &&  att2->att().movementAtt().mapID() != PlayerPool_ReservationMap() ) {
								...
								if (att1->att().movementAtt().mapID() == PlayerPool_ReservationMap()) {
									struct Component *component = PlayerEntity_Component(entity1);
								...
								if (att2->att().movementAtt().mapID() == PlayerPool_ReservationMap()) {
									struct Component *component = PlayerEntity_Component(entity2);
									const MovementAtt *movementAtt = Movement_Att(component->movement);
									int32_t nextMap = movementAtt->prevNormalMap();
									Movement_BeginChangeScene(component->movement, nextMap, &movementAtt->prevCoord());
								}
...
static vector<RecordInfo>* CurRank(NetProto_Rank::Type type) {
	...
	else if (type == NetProto_Rank::PVPSCORE)
		cur = &pool.rankPvp;
...
static void FirstDayReLoadRank(void *arg) {
	...
	DCProto_InitRank rankPvp;
	rankPvp.set_type(NetProto_Rank::PVPSCORE);
	GCAgent_SendProtoToDCAgent(&rankPvp, false);
...
static void SendWeekPKAwardCP(void *arg) {
	bool v = *(bool *)arg;
	// delete (bool *)arg;
	PlayerPool_SendWeekPKAward(v);
}

static void SendPKAwardCP(void *arg) {
	bool v = *(bool *)arg;
	// delete (bool *)arg;
	PlayerPool_SendPKAward(v);
}
...
static void ResetAllPvpScoreCP(void *arg) {
	bool v = *(bool *)arg;
	// delete (bool *)arg;
	PlayerPool_ResetPvpScore(v);
}
...
//	Timer_AddEvent(Time_TimeStamp() + 60 * 5, -1, Temp, NULL);
	...
	h[0] = 0;
	Timer_AddEvent(2, h, 1, ResetAllPvpScoreCP, new bool(true), "ResetAllPvpScore");
	...
	Timer_AddEvent(-1, dayEnd, 1, SendPKAwardCP, new bool(true), "DailyPK");
	Timer_AddEvent(2, dayEnd, 1, SendWeekPKAwardCP, new bool(true), "WeeklyPK");
	for (int i = 0; ; i++) {
...

	{
		...
		Timer_AddEvent(next, 60, AutoRecoverDurability, NULL, "AutoRecoverDurability");
	}
	...
	DCProto_InitRank rankPvp;
	rankLevel.set_type(NetProto_Rank::PVPSCORE);
	GCAgent_SendProtoToDCAgent(&rankPvp, false);
	GCAgent_WaitDCAgent(1);
...
void PlayerPool_InitRank(DCProto_InitRank* rank) {
	...
	} else if (rank->type() == NetProto_Rank::PVPSCORE) {
		pool.rankPvp.clear();
		pool.rankPvpInfo.clear();
		for (int64_t i = 0; i < rank->rank_size(); ++i) {
			pool.rankPvp.push_back(rank->rank(i));
			pool.rankPvpInfo[rank->rank(i).role().roleID()] = i;
		}
...
int64_t PlayerPool_QueryRank(const int64_t roleID, NetProto_Rank* rank) {
	...
	if (rank->type() == NetProto_Rank::POWER) {
		...
		*info = pool.rankWorldBossFinal;
		...
		for (int i = rank->range(); i < rank->range() + 6 && i < (int)pool.rankPvp.size(); ++i) {
			RecordInfo* info = rank->add_rank();
			*info = pool.rankPvp[i];
		}
		map<int64_t, int64_t>::iterator it = pool.rankPvpInfo.find(roleID);
		if (it == pool.rankPvpInfo.end()) {
			rank->mutable_self()->mutable_role()->set_roleID(-1);
		} else {
			*rank->mutable_self() = pool.rankPvp[it->second];
			rank->mutable_finalKiller()->set_arg1(it->second);
		}
		rank->mutable_finalKiller()->mutable_role()->set_roleID((int)pool.rankPvp.size());
	} 
...
{
	return pool.rankGodCur.find(roleId)->second;
}
...
void PlayerPool_SynPvpInfoTime(int32_t preTime, bool flag) {
	pool.rankPvpRecordsTime = preTime;
	if (flag) {
		DCProto_SaveSingleRecord save;
		save.set_mapID(-100);
		save.mutable_record()->set_arg1(pool.rankPvpRecordsTime);
		GCAgent_SendProtoToDCAgent(&save);
	}
}
...
void PlayerPool_ResetPvpScore(bool flag) {
	static vector<RecordInfo> resetRankPvp;
	static int64_t indexPvp;
	if (flag) {
		resetRankPvp.clear();
		resetRankPvp = pool.rankPvp;
		indexPvp = 0;
	}
	
	if (indexPvp < (int64_t)resetRankPvp.size()) {
		struct PlayerEntity* entity = PlayerEntity_PlayerByRoleID(resetRankPvp[indexPvp].role().roleID());
		while (entity != NULL) {
			struct Component *component = PlayerEntity_Component(entity);
			Item_ResetPvpScore(component->item);
			...
			entity = PlayerEntity_PlayerByRoleID(resetRankPvp[indexPvp].role().roleID());
		}

		if (entity == NULL) {
			static DCProto_PingPongAward proto;
			proto.Clear();
			proto.set_type(NetProto_Rank::PVPSCORE);
			proto.set_roleID(resetRankPvp[indexPvp].role().roleID());
			GCAgent_SendProtoToDCAgent(&proto);
			indexPvp++;
		}
...
void PlayerPool_SendPKAward(bool flag) {
	...
	static int64_t index;
	if (flag) {
		if (Time_TheSameDay(pool.rankPvpRecordsTime, Time_TimeStamp())) {
			return;	
		}

		PlayerPool_SynPvpInfoTime(Time_TimeStamp(), true);
		rankAward.clear();
		rankAward = pool.rankPvp;
		index = 0;

		DEBUG_LOGRECORD("PlayerPool_SendPKAward begin: %d", (int)Time_TimeStamp());
	}

	if (index < (int64_t)rankAward.size()) {
		int range = Config_AwardRange(index, rankAward.size());
		const AwardInfo *award = AwardInfoManager_AwardInfo(AwardInfo::DAILY_PK, range);
		assert(award != NULL);
		...
		while (entity != NULL) {
			...
			index++;
			if (index >= (int64_t)rankAward.size()) {
				DEBUG_LOGRECORD("PlayerPool_SendPKAward end: %d", (int)Time_TimeStamp());
				return;
			}

			range = Config_AwardRange(index, rankAward.size());
			award = AwardInfoManager_AwardInfo(AwardInfo::DAILY_PK, range);
			assert(award != NULL);
			entity = PlayerEntity_PlayerByRoleID(rankAward[index].role().roleID());
		}

		if (entity == NULL) {
			static DCProto_PKAward proto;
			proto.Clear();
			proto.set_roleID(rankAward[index].role().roleID());
			proto.set_range(range);
			GCAgent_SendProtoToDCAgent(&proto);
			index++;
		}
	} else {
...
void PlayerPool_SendWeekPKAward(bool flag) {
	...
	if (flag) {
		...
		PlayerPool_SynPvpInfoTime(Time_TimeStamp(), true);
		rankAwardWeek.clear();
		rankAwardWeek = pool.rankPvp;
		indexWeek = 0;
	}

	if (indexWeek < (int64_t)rankAwardWeek.size()) {
		int range = Config_AwardRange(indexWeek, rankAwardWeek.size());
		const AwardInfo *award = AwardInfoManager_AwardInfo(AwardInfo::WEEKLY_PK, range);
		assert(award != NULL);
		...
		while (entity != NULL) {
			...
			range = Config_AwardRange(indexWeek, rankAwardWeek.size());
			award = AwardInfoManager_AwardInfo(AwardInfo::WEEKLY_PK, range);
			assert(award != NULL);
			entity = PlayerEntity_PlayerByRoleID(rankAwardWeek[indexWeek].role().roleID());
		}

		if (entity == NULL) {
			static DCProto_WeekPKAward proto;
			proto.Clear();
			proto.set_roleID(rankAwardWeek[indexWeek].role().roleID());
			proto.set_range(range);
			GCAgent_SendProtoToDCAgent(&proto);
			indexWeek++;
		}
...
static void SendReservationAward(int64_t roleID, int rmb, int flag) {
	int index = -1;
	int rmb_s = 0;
	if (rmb == 10000) {
		if (flag == 1) {
	...
	const AwardInfo *award = AwardInfoManager_AwardInfo(AwardInfo::YUEZHAN, index);
	if (flag != 0 && award != NULL) {
		struct PlayerEntity* entity = PlayerEntity_PlayerByRoleID(roleID);
		if (entity != NULL) {
			if (flag == 0) {
				PlayerEntity_AddMail(entity, ItemInfo::NONE, award->award(), 1, award->name().c_str(), award->content().c_str(), rmb, true);
			}else {
				PlayerEntity_AddMail(entity, ItemInfo::GOODS, award->award(), 1, award->name().c_str(), award->content().c_str(), rmb_s, flag == 1 ? true : false);
			}
		}else {
			static DCProto_SendMail dsm;
			dsm.Clear();
			dsm.mutable_sm()->set_receiver(roleID);
			dsm.mutable_sm()->mutable_mail()->set_title(award->name());
			dsm.mutable_sm()->mutable_mail()->set_sender(Config_Words(1));
			dsm.mutable_sm()->mutable_mail()->set_content(award->content());
			if (flag == 0) {
				dsm.mutable_sm()->mutable_mail()->mutable_item()->set_type(PB_ItemInfo::NONE);
				dsm.mutable_sm()->mutable_mail()->set_rmb(rmb);
				dsm.mutable_sm()->mutable_mail()->set_isRmb(true);
			}else {
				dsm.mutable_sm()->mutable_mail()->mutable_item()->set_type(PB_ItemInfo::GOODS);
				dsm.mutable_sm()->mutable_mail()->set_rmb(rmb);
				dsm.mutable_sm()->mutable_mail()->set_isRmb(true);
...
static int32_t Negotiation(struct PlayerEntity * entity, NetProto_Reservation * info, int type) {
	...
	if (type == 1 || type == 2) {
		...
		if (type == 1) {
			...
			if (entity2 != NULL) {
				...
				int id = 200 + index; 
				queryRole.set_id(-id); //-200 -219
				queryRole.set_roleID(info->roleID());
		...
		int rmb =  Config_ReservationCost(pool.reservation[index].rmb, false);
		struct Component *component = PlayerEntity_Component(entity);
		if (component == NULL) 
			return -16;
...
int PlayerPool_Reservation(struct PlayerEntity * entity, NetProto_Reservation * info) {
	...
	if (info->roleID() < 0) {
		if (info->roleID() == -1) {
			...
			if (Time_TimeStamp() >= begin && Time_TimeStamp() <= end) {
				if ( 0 == Negotiation(entity, info, 2) )
					return 0;
			}
			return -13;
			/*			
			...
			*/
			...
			int end = Config_ReservationTime(hour, min);
			if (Time_TimeStamp() >= begin && Time_TimeStamp() <= end) {
				if ( 0 == Negotiation(entity, info, 3) )
					return 0;
			}
			return -13;
			...
			int end = Config_ReservationTime(hour, min);
			if (Time_TimeStamp() >= begin && Time_TimeStamp() <= end) {
				if ( 0 == Negotiation(entity, info, 4) )
					return 0;
			}
			return -131;
			...
			int end = Config_ReservationTime(hour, min);
			if (Time_TimeStamp() >= begin && Time_TimeStamp() <= end) {
				if ( 0 == Negotiation(entity, info, 5) )
					return 0;
			}
			return -132;
		}
	...
	*/
		...
		if (Time_TimeStamp() >= begin && Time_TimeStamp() <= end) {
			if ( 0 == Negotiation(entity, info, 1) )
				return 0;
		}
		return -26;
	/*
...
void PlayerPool_ReservationRefresh(int index, int64_t roleID) {
	if (index == -1) {
		...
		for (int i = 0; i < 8; ++i) {
			if (pool.reservation[i].roleID1 != -100) {
				if (pool.reservation[i].powerType == 1) {
					...
					pool.reservation[i].name2 = "";
					pool.reservation[i].roleID2 = -100;
					pool.reservation[i].type2 = PB_ProfessionInfo::NPC;
				}
			}
		}
		return;
		...
		int min = 0;
		for (int i = 0; i < 20; ++i) {
			int res = ( pool.reservation[i].powerType % 1000 ) / 100;
			if ( res == 1 || res == 2 ) {
				return;
			}
			...
			if (Time_TimeStamp() >= begin && Time_TimeStamp() <= end) {
				if ( pool.reservation[i].roleID1 == roleID || pool.reservation[i].roleID2 == roleID ) {
					if ( pool.reservation[i].powerType == 21 || pool.reservation[i].powerType == 31 ) {
						...
						}else if ( pool.reservation[i].roleID2 == roleID ) {
							pool.reservation[i].powerType += 200;
						}

						int win = ( pool.reservation[i].powerType % 1000 ) / 100;
						if (win == 1) {
							SendReservationAward(pool.reservation[i].roleID1, pool.reservation[i].rmb, 1);
							for (set<int64_t>::iterator it = pool.reservation[i].setRole1.begin(); it != pool.reservation[i].setRole1.end(); ++it) {
								SendReservationAward(*it, 50, 1);
							}
							
							SendReservationAward(pool.reservation[i].roleID2, pool.reservation[i].rmb, -1);
...
/*
						if ( pool.reservation[i].roleID1 == roleID ) {
							DEBUG_LOG("QQQQQQQQQQQQQQQQQQQQQQQQQ");
							SendReservationAward(pool.reservation[i].roleID1, pool.reservation[i].rmb, 1);
							for (set<int64_t>::iterator it = pool.reservation[i].setRole1.begin(); it != pool.reservation[i].setRole1.end(); ++it) {
								SendReservationAward(*it, 50, 1);
							}
		...
		*/
		...
		if ( res != 1 && res != 2 ) {
			...
			if (entity1 != NULL && entity2 != NULL) {
				...
				{
					//TODO
					int type = (pool.reservation[index].powerType % 1000) / 100;
					if (type == 1) {
						if (att2->att().movementAtt().mapID() == PlayerPool_ReservationMap()) {
							struct Component *component = PlayerEntity_Component(entity2);
							const MovementAtt *movementAtt = Movement_Att(component->movement);
			...
			if ((pool.reservation[index].powerType / 100) % 10 == 1) {
				SendReservationAward(pool.reservation[index].roleID1, pool.reservation[index].rmb, 1);
				for (set<int64_t>::iterator it = pool.reservation[index].setRole1.begin(); it != pool.reservation[index].setRole1.end(); ++it) {
					SendReservationAward(*it, 50, 1);
				}
				SendReservationAward(pool.reservation[index].roleID2, pool.reservation[index].rmb, -1);
				for (set<int64_t>::iterator it = pool.reservation[index].setRole2.begin(); it != pool.reservation[index].setRole2.end(); ++it) {
					SendReservationAward(*it, 50, -1);
				}
			}else if ((pool.reservation[index].powerType / 100) % 10 == 2) {
				SendReservationAward(pool.reservation[index].roleID1, pool.reservation[index].rmb, -1);
				for (set<int64_t>::iterator it = pool.reservation[index].setRole1.begin(); it != pool.reservation[index].setRole1.end(); ++it) {
					SendReservationAward(*it, 50, 1);
				}
				...
				SendReservationAward(pool.reservation[index].roleID1, pool.reservation[index].rmb, 0);
				for (set<int64_t>::iterator it = pool.reservation[index].setRole1.begin(); it != pool.reservation[index].setRole1.end(); ++it) {
					SendReservationAward(*it, 50, 0);
				}
				SendReservationAward(pool.reservation[index].roleID2, pool.reservation[index].rmb, 0);
				for (set<int64_t>::iterator it = pool.reservation[index].setRole2.begin(); it != pool.reservation[index].setRole2.end(); ++it) {
					SendReservationAward(*it, 50, 0);
				}
			}
...
bool PlayerPool_ReservationEnterOrPower(int64_t roleID, int op) {
	...
	*/
	...
	for (int index = 0; index < 20; ++index) {
		...
		int endFight = Config_ReservationTime(hour, min);
		
		if (op == 1) {
			if (Time_TimeStamp() >= begin && Time_TimeStamp() <= end) {
				if ( pool.reservation[index].roleID1 == roleID || pool.reservation[index].roleID2 == roleID	 ) {
					if (pool.reservation[index].powerType < 100) {
						return true;
					}
					return false;
				}
			...
			if (Time_TimeStamp() >= end && Time_TimeStamp() <= endFight) {
				if ( pool.reservation[index].roleID1 == roleID || pool.reservation[index].roleID2 == roleID	 ) {
					if (pool.reservation[index].powerType < 100) {
						return true;
					}
...
void PlayerPool_ReservationQueryRole(DCProto_QueryRole * info) {
	...
	int index = (-info->id()) % 100;

	pool.reservation[index].roleID2 = info->att().att().baseAtt().roleID(); // nedd name and profession
	pool.reservation[index].name2 = info->att().att().baseAtt().name();
	pool.reservation[index].type2 = (PB_ProfessionInfo::Type)info->att().att().baseAtt().professionType();
}
