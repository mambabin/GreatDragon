...
class ItemPackage{
	...
	public:
		...
		inline int32_t openServerScore() const {
			return openServerScore_;
		}
		inline void set_openServerScore(int32_t value) {
			openServerScore_ = value;
		}

		inline bool pkStatus() const {
			return pkStatus_;
		}
		inline void set_pkStatus(bool value) {
			pkStatus_ = value;
		}
	...
	private:
		...
		GodShipAsset godShips_[506];

		int32_t godShipsPackage_[500];

		int64_t ridesFood_;

		int32_t openServerScore_;
