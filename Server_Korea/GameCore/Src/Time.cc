...
int Time_RandomEx(int begin, int end)
{
	if(begin == end)
		return begin;
	...
	if(randomSeed > 1900000000)
		randomSeed = 0;

	return begin + v % (end - begin);
}
...
int32_t Time_TodayZeroTime(int32_t clock) {
	time_t clock__t = clock;
	struct tm clock_tm;
	Time_LocalTime(clock__t, &clock_tm);
	clock_tm.tm_hour = 0;
	clock_tm.tm_min = 0;
	clock_tm.tm_sec = 0;
	return  mktime(&clock_tm);
}
