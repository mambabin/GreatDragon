...
void GCMsgProcessor_ProcessNetMsg(zmq::message_t *msg) {
	...
	if(player != NULL)
		PlayerEntity_AddMsgSt(player, groupID, unitID);
						...
						case NetProto_ClearRoom::UNITID:
							{
								...
								if (ret != 0) {
									DEBUG_LOGERROR("Failed to clearroom, check, %d", ret);
									break;
								}
								...
								if ((info->id() >= Config_SingleBegin() && info->id() <= Config_SingleEnd())
									...
									MapInfoManager_UpdateSingleRecord(info->id(), clearRoom.totalTime(), entity);
									DEBUG_LOG("Clear Room begin");
								}
						...
						case NetProto_Inherit::UNITID:
							{
								...
								if (PlayerEntity_Inherit(player, proto.parentPos() == NetProto_Inherit::BODY, proto.parentID(), proto.childPos() == NetProto_Inherit::BODY, proto.childID(), proto.useRMB()) == -1)
									break;
						...
						case NetProto_StrongBaseWing::UNITID:
							{
								...
								if (Fight_StrongBaseWing(component->fight, proto.useRMB()) == -1)
									break;
						...
						case NetProto_Rides::UNITID:
							{
								...
								int re = Equipment_Ride(component->equipment, proto.rides());
								if(re < 0)
								{
						...
						case NetProto_BuyItem::UNITID:
							{
								...
								if (cur->type() == PB_ItemInfo::EQUIPMENT) {
									...
									if (business->currencyType() == BusinessInfo::RMB)
										rmb = equip->rmb() * cur->count() / Config_Divisor();
									else if (business->currencyType() == BusinessInfo::SUB_RMB)
										subRMB = equip->rmb() * cur->count() / Config_Divisor();
									else if (business->currencyType() == BusinessInfo::MONEY)
										price = equip->price() * cur->count() / Config_Divisor();
									else if (business->currencyType() == BusinessInfo::LOVE_POINT)
										lovePoint = equip->lovePoint() * cur->count() / Config_Divisor();
									else if (business->currencyType() == BusinessInfo::PVP_SCORE)
										pvpScore = equip->pvpScore() * cur->count() / Config_Divisor();
									else if (business->currencyType() == BusinessInfo::GOD_SCORE)
										godScore = equip->godScore() * cur->count() / Config_Divisor();
									else if (business->currencyType() == BusinessInfo::FACTION_CONTRIBUTE)
										factionContribute = equip->factionContribute() * cur->count() / Config_Divisor();
									else
										break;
								} else if (cur->type() == PB_ItemInfo::GOODS) {
									const GoodsInfo *info = GoodsInfoManager_GoodsInfo(cur->id());
									if (business->currencyType() == BusinessInfo::RMB)
										rmb = info->rmb() * cur->count() / Config_Divisor() * buyItem.count();
									else if (business->currencyType() == BusinessInfo::SUB_RMB)
										subRMB = info->rmb() * cur->count() / Config_Divisor() * buyItem.count();
									else if (business->currencyType() == BusinessInfo::MONEY)
										price = info->price() * cur->count() / Config_Divisor() * buyItem.count();
									else if (business->currencyType() == BusinessInfo::LOVE_POINT)
										lovePoint = info->lovePoint() * cur->count() / Config_Divisor() * buyItem.count();
									else if (business->currencyType() == BusinessInfo::PVP_SCORE)
										pvpScore = info->pvpScore() * cur->count() / Config_Divisor() * buyItem.count();
									else if (business->currencyType() == BusinessInfo::GOD_SCORE)
										godScore = info->godScore() * cur->count() / Config_Divisor() * buyItem.count();
									else if (business->currencyType() == BusinessInfo::FACTION_CONTRIBUTE)
										factionContribute = info->factionContribute() * cur->count() / Config_Divisor() * buyItem.count();
									else
										break;
								} else if (cur->type() == PB_ItemInfo::FASHION) {
									if (Item_BuyFashion(component->item, cur->id(), buyItem.count() > 1, (float)cur->count() / Config_Divisor(), business->currencyType()) != 0)
										break;
								} else if (cur->type() == PB_ItemInfo::WING) {
									if (Item_BuyWing(component->item, cur->id(), buyItem.count() > 1, (float)cur->count() / Config_Divisor(), business->currencyType()) != 0)
										break;
								} else {
									assert(0);
								}
								...
								if (rmb > 0)
									Item_ModifyRMB(component->item, -rmb, false, arg1, cur->id(), cur->count());
								if (subRMB > 0) {
									Item_ModifyRMB(component->item, -subRMB, false, arg1, cur->id(), cur->count());
								}
								if (lovePoint > 0) {
