	...
	for(int i = 0; i < MSGST_A; i++)
	{
		for(int j = 0; j < MSGST_B; j++)
		{
			if(player->msgSt[i][j] > 15)
				DEBUG_LOGERROR("role:%ld, msgst->%d, %d, %d", player->att.att().baseAtt().roleID(), i, j, player->msgSt[i][j]);
...
void PlayerEntity_AddMsgSt(struct PlayerEntity *player, uint32_t group, uint32_t unit)
{
	if (!PlayerEntity_IsValid(player))
		return;
...
int64_t PlayerEntity_AddMail(struct PlayerEntity *entity, ItemInfo::Type type, int64_t id, int count, const char *title, const char *content, int subrmb) {
	if (!PlayerEntity_IsValid(entity))
		return -1;
...
int PlayerEntity_Inherit(struct PlayerEntity *entity, bool parentBody, int32_t parentID, bool childBody, int32_t childID, bool useRMB) {
	if (!PlayerEntity_IsValid(entity))
		return -1;
	...
	if (parent->type() != child->type())
		return -1;
	if (parentAsset->strongLevel() < childAsset->strongLevel())
		return -1;
	...
	childAsset->set_strongLevel(parentAsset->strongLevel());
	...
	if (Config_ScribeHost() != NULL) {
		...
		index += strlen(index);
		SNPRINTF2(buf, index, "\",\"parentEquipStrongLevel\":\"%d", parentAsset->strongLevel());
		index += strlen(index);
		SNPRINTF2(buf, index, "\",\"childEquipStrongLevel\":\"%d", childAsset->strongLevel());
		index += strlen(index);
	}
	for (int i = 0; i < EquipAsset::ENHANCEDELTA_SIZE; i++) {
		if (parentAsset->enhanceDelta(i) > childAsset->enhanceDelta(i)) {
			childAsset->set_enhanceDelta(i, parentAsset->enhanceDelta(i));
			if (Config_ScribeHost() != NULL) {
				SNPRINTF2(buf, index, "\",\"parentEquipAttr%d\":\"%d", i + 1, parentAsset->enhanceDelta(i));
				index += strlen(index);
...
int PlayerEntity_MoneyTree(struct PlayerEntity* entity, NetProto_MoneyTree* info) {
	...
	if (info->index() == 0) {
		...
		int freeCount = entity->att.dayEvent(35) & 0xFF00;
		if (freeCount >= 0x0300) {
			return -4;
		}

		Item_ModifyMoney(entity->component.item, level * money * randNum);
	...
	item->set_count(level * money * randNum);
	GCAgent_SendProtoToClients(&entity->id, 1, &gr);
