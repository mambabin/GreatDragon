...
#include <map>
#include <MD5.hpp>
#include "../../SDK/KOREA/NAVER/lib/include/MACManager.h"
...
static size_t CB_CheckUser(void *ptr, size_t size, size_t nmemb, void *userdata) {
	...
	static size_t CB_CheckRecharge(void *ptr, size_t size, size_t nmemb, void *userdata) {
		...
		else if(unit->recharge.info().platform() == "Korea_Naver")
		{
			static char v[CONFIG_FIXEDARRAY];
			if(ConfigUtil_ReadStrFromJson(unit->buf.c_str(), "code", v, sizeof(v)))
			{
				if(atoi(v) == 0)
					status = 1;
				else
		...
		else if(unit->recharge.info().platform() == "Korea_TStore")
		{
			static char v[CONFIG_FIXEDARRAY];
			if(ConfigUtil_ReadStrFromJson(unit->buf.c_str(), "status", v, sizeof(v)))
			{
		...
		if (status == 0) {
			...
			if (status == 1) {
				...
				else if(unit->recharge.info().platform() == "Korea_Naver")
				{
					...
					unit->recharge.set_rmb(v);
					DCAgent_SendProtoToGCAgent(&unit->recharge);
					DEBUG_LOG("naver successful to recharge->%d, %s", v, unit->recharge.recharge().order().c_str());
				}
			}
			else if(unit->recharge.info().platform() == "Korea_TStore")
			{
				...
				unit->recharge.set_rmb(v);
				DCAgent_SendProtoToGCAgent(&unit->recharge);
				DEBUG_LOG("tstore successful to recharge->%d, %s", v, unit->recharge.recharge().order().c_str());
			}
			else if (status == -1) 
			{
	...
	int Web_CheckRecharge(const DCProto_Recharge *proto) {
		...
		else if (proto->info().platform() == "Korea_Naver")
		{
			const std::string KEY = "AD_1004522_DumCZIiIWNKL7uvSwhX06gTjhQWPYP460xh7Lf3sA2k8gPwqjOkB9E1Y6zYaUmN5";
			const std::string URL = "http://dev.apis.naver.com/AD_1004522/hmac/hmactest.html";
			...
			static char data[CONFIG_FIXEDARRAY];
			snprintf(data, sizeof(data), "%s?nonce=%s&paymentSeq=%s", URL.c_str(), str, str1);

			MACManager manager(KEY);
			std::string encryptedUrl = manager.getEncryptUrl(data);

			curl_easy_setopt(unit->curl, CURLOPT_URL, data);
		}
		else if(proto->info().platform() == "Korea_TStore")
		{
			const std::string URL = "https://iapdev.tstore.co.kr/digitalsignconfirm.iap";
			//const std::string URL = "https://iap.tstore.co.kr/digitalsignconfirm.iap";
			unit = new struct Unit;
			unit->done = false;
			unit->recharge = *proto;
			unit->curl = curl_easy_init();
			...
			static char data[CONFIG_FIXEDARRAY];
			strcpy(data, URL.c_str());
			curl_easy_setopt(unit->curl, CURLOPT_URL, data);
			//snprintf(data, sizeof(data), "%s?txid=%s&appid=%s&signdata=%s", URL.c_str(), proto->recharge().product().c_str(), "OA00686519", proto->recharge().order().c_str());
			snprintf(data, sizeof(data), "{ \"txid\":\"%s\", \"appid\":\"%s\", \"signdata\":\"%s\"", proto->recharge().product().c_str(), "OA00686519", proto->recharge().order().c_str());

			curl_easy_setopt(unit->curl, CURLOPT_POST, 1);
			curl_easy_setopt(unit->curl, CURLOPT_POSTFIELDS, data);
