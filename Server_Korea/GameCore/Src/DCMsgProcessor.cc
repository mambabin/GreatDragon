...
void DCMsgProcessor_ProcessDCMsg(zmq::message_t *msg) {
	...
	DEBUG_LOGERROR("dc msg->%d, %d", groupID, unitID);
							...
							if (recharge.over()) {
								...
								if (Web_CheckRecharge(&recharge) == 1) {
									...
									if (ret >= 0) {
										...
										int re = DataManager_RechargeOver(recharge.recharge().order().c_str(), &recharge.info(), recharge.roleID(), recharge.level());
										if(re == 0)
										{
		...
		case DCProto_InitRank::GROUPID:
			{
				switch(unitID) {
					...
					case DCProto_PingPongAward::UNITID:
						{
							...
							}else if (proto.type() == NetProto_Rank::PVPSCORE) {
								static PB_PlayerAtt att;
								att.Clear();
								if (DataManager_LoadRoleData(proto.roleID(), &att)) {
									att.mutable_itemPackage()->set_pkScore(1000);
									att.mutable_itemPackage()->set_pkStatus(false);
									DataManager_SaveRoleData(&att);
								}
					...
					case DCProto_PKAward::UNITID:
						{
							static DCProto_PKAward proto;
							if (!proto.ParseFromArray(data, size))
								break;

							const AwardInfo *award = AwardInfoManager_AwardInfo(AwardInfo::DAILY_PK, proto.range());
							if (award != NULL) {
								static PB_PlayerAtt att;
								if (DataManager_LoadRoleData(proto.roleID(), &att)) {
									if (att.mutable_itemPackage()->pkStatus()) {
										static PB_MailInfo mail;
										mail.set_title(award->name());
										mail.set_sender(Config_Words(1));
										mail.set_content(award->content());
					...
					case DCProto_WeekPKAward::UNITID:
						{
							static DCProto_WeekPKAward proto;
							if (!proto.ParseFromArray(data, size))
								break;

							const AwardInfo *award = AwardInfoManager_AwardInfo(AwardInfo::WEEKLY_PK, proto.range());
							if (award != NULL) {
								static PB_PlayerAtt att;
								if (DataManager_LoadRoleData(proto.roleID(), &att)) {
									if (att.mutable_itemPackage()->pkStatus()) {
